using CustomInfoSystem;
using ProgressionSystem.Quest.Implementations;
using System;
using UnityEngine;

namespace ProgressionSystem.Quest.Testing
{
    public class ResourceRewardInfo : ICustomInfo
    {
        private readonly ResourceReward _reward;
        private readonly ResourceRewardDisplayerMB _displayerPrefab;

        public ResourceRewardInfo(ResourceReward reward, ResourceRewardDisplayerMB displayerPrefab)
        {
            _reward = reward ?? throw new ArgumentNullException(nameof(reward));
            _displayerPrefab = displayerPrefab != null ? displayerPrefab : throw new ArgumentNullException(nameof(displayerPrefab));
        }

        public GameObject CreateInfoObject()
        {
            ResourceRewardDisplayerMB displayer = GameObject.Instantiate(_displayerPrefab);
            displayer.DisplayReward(_reward);

            return displayer.gameObject;
        }
    }
}